...
std::string ProtocolHandler::token;
...
std::string ProtocolHandler::handleMessage(std::string message, Device& device)
{
	...
	if (messageType == "SET")
	{
		if (numOfComponent != 4)
			return MESSAGE_INVALID_FORMAT;
		return handleSetMessage(messageComponents[2], messageComponents[3], device);
	}
	else if (messageType == "ADDTIME_P")
	{
		...
		return handleAddTimeMessage(messageComponents[2], time, device);
	}
	else if (messageType == "ADDTIME_R")
	{
		...
		if (!isTimeRange(messageComponents[2], timerange))
			return ADDTIME_P_INVALID_FORMAT_VALUE;
		return handleAddTimeRangeMessage(messageComponents[2], timerange, device);
	}
	else if (messageType == "RMVTIME_P")
	{
		...
		return handleRmvTimeMessage(timeId, device);
	}
	else if (messageType == "RMVTIME_R")
	{
		...
		return handleRmvTimeRangeMessage(timerangeId, device);
	}
	else if (messageType == "GET_I")
	{
		if (numOfComponent != 3)
			return MESSAGE_INVALID_FORMAT;
		return handleGetEnvironmentMessage(messageComponents[2], device);
	}
	else if (messageType == "GET_D")
	{
		if (numOfComponent != 3)
			return MESSAGE_INVALID_FORMAT;
		return handleGetDeviceParamMessage(messageComponents[2], device);
	}
	else if (messageType == "COMMAND")
	{
		...
		return handleCommandMessage(messageComponents[2] == "ON", device);
	}
	else if (messageType == "GET_PARAM")
	{
		...
		if (numOfComponent != 2)
			return MESSAGE_INVALID_FORMAT;
		return handleGetAllParamMessage(device);
	}
	return MESSAGE_TYPE_NOT_EXIST;
}
